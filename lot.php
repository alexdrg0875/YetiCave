...
if($con == false) {
  ...
  $query_result = mysqli_query($con, "SELECT l.id, l.title AS name, c.NAME AS category, l.price, l.path AS image_path, l.alt_title AS alt, l.description FROM lots AS l JOIN categories AS c ON l.category_id = c.id WHERE l.id = $_GET[id]");
  if (!$query_result){
    print('Ошибка MYSQL:' . mesqli_error());
  } else {
