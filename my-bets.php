...
if($connect_sql == false) {
  ...
  $query_result = mysqli_query($connect_sql, "SELECT l.id FROM lots AS l JOIN categories AS c ON l.category_id = c.id JOIN bets AS b ON b.lot_id = l.id WHERE b.user_id = $user_id GROUP BY b.lot_id");
  if (!$query_result){
    print('Ошибка MYSQL:' . mysqli_error($connect_sql));
  } else {
...
$page_content = renderTemplate('templates/my-bets.php', [
  'lots' => $lots,
